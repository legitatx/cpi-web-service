...
public class DeleteLayout implements Handler {
    ...
    public void handle(@NotNull Context ctx) throws Exception {
        ...
        APICore.getLogger().info("Successfully verified and decoded ID token for user: " + uid + " (DeleteLayout)");
        ...
        if (document.exists()) {
            ...
            if (layouts != null) {
                for (int i = 0; i < layouts.size(); i++) {
                    JsonObject layoutJson = parser.parse(gson.toJson(layouts.get(i))).getAsJsonObject();
                    if (Long.valueOf(layoutJson.get("layoutId").getAsString()) == layoutId) {
                        layouts.remove(i);
                        break;
                    }
                }
                ApiFuture<WriteResult> updateEquipment = docRef.update("layouts", layouts);
                APICore.getLogger().info("Updated deleted layout " + layoutId + " for user " + uid + " at: " + updateEquipment.get().getUpdateTime());
            }
        }
    }
}
