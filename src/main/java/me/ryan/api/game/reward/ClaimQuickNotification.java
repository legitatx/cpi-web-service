...
public class ClaimQuickNotification implements Handler {

    public ClaimQuickNotification() {
    }
    ...
    public void handle(@NotNull Context ctx) throws Exception {
        ...
        APICore.getLogger().info("Successfully verified and decoded ID token for user: " + uid + " (ClaimQuickNotification)");
        ...
        Reward quickNotificationReward = new Reward.Builder().withCoins(30).build();

        if (document.exists()) {
            APICore.getRewardManager().saveReward(document, quickNotificationReward, null);
        }

        JSONObject finalResponse = new JSONObject();
        finalResponse.put("reward", APICore.getRewardManager().getRewardAsJSON(quickNotificationReward));
