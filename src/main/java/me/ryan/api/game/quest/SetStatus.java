...
public class SetStatus implements Handler {
    ...
    public void handle(@NotNull Context ctx) throws Exception {
        ...
        APICore.getLogger().info("Successfully verified and decoded ID token for user: " + uid + " (QuestSetStatus)");

        Firestore db = APICore.getDatabaseManager().firebase().getFirestore();

        String questName = ctx.path().split("/")[3];
        int status = Integer.parseInt(ctx.body());
        QuestStatus questStatus = QuestStatus.values()[status];

        DocumentReference docRef = db.collection("users").document(uid);
        ApiFuture<DocumentSnapshot> future = docRef.get();
        DocumentSnapshot document = future.get();
        if (document.exists()) {
            JSONObject questData = APICore.getQuestManager().setQuestStatus(docRef, document, questName, questStatus);
            ...
            JSONObject questResponse = generateQuestResponse(wsEvents, questName, finalQuestData, decodedToken, reward, docRef);
            ...
            APICore.getLogger().info(questResponse.toString());

            ctx.result(questResponse.toString());
        } else {
            APICore.getLogger().severe(uid + " - " + "(document.exists() == false) (SetStatus)");

            Sentry.getContext().addExtra("halted", true);
            Sentry.capture("(document.exists() == false) (SetStatus)");
    ...
    private JSONObject generateQuestResponse(JSONObject wsEvents, String questId, String questData, FirebaseToken token, JSONObject rewardAsJson, DocumentReference reference) throws Exception {
